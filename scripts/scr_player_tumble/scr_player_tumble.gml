function scr_player_tumble()
{
    ...
    if (floor(image_index) == (image_number - 1) && sprite_index == spr_player_machroll && movespeed > 12)
    {
    ...
    if (input_buffer_jump < 8 && grounded && hsp != 0 && sprite_index == spr_tumble && !place_meeting(x, y, obj_pinballlauncher) && !place_meeting(x, y, obj_bigcheese))
    {
        if (!scr_solid(x, y - 16))
        {
    ...
    if (!key_down && key_attack && grounded && (sprite_index != spr_tumble && sprite_index != spr_tumbleend) && !scr_solid(x, y - 16) && !scr_solid(x, y - 32) && sprite_index != spr_player_breakdance)
    {
        if (crouchslipbuffer == 0)
        {
