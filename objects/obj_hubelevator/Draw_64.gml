if (state == states.titlescreen)
{
    var w = 0;
    var h = 0;
    var pad = 16;
    var yy = 0;
    
    for (var i = 0; i < array_length(hub_array); i++)
    {
        var txt = hub_array[i][1];
        
        if (string_width(txt) > w)
            w = string_width(txt);
        
        h += string_height(txt);
    }
    
    w += pad;
    h += pad;
    yy = (obj_screensizer.actual_height / 2) + (h / 2);
    draw_rectangle_color((obj_screensizer.actual_width / 2) - (w / 2), (obj_screensizer.actual_height / 2) - (h / 2), (obj_screensizer.actual_width / 2) + (w / 2), yy, c_black, c_black, c_black, c_black, false);
    yy -= (pad / 2);
    draw_set_valign(fa_bottom);
    draw_set_halign(fa_center);
    
    for (var i = 0; i < array_length(hub_array); i++)
    {
        var c = (selected == i) ? 16777215 : 8421504;
        var txt = hub_array[i][1];
        draw_text_color(480, yy, txt, c, c, c, c, 1);
        yy -= string_height(txt);
    }
}
