...
if (place_meeting(x, y, obj_player) && (state == states.revolver || state == states.dynamite || state == states.throwing || state == states.estampede) && grounded && vsp > 0 && !obj_player1.hurted)
{
    ...
    state = states.punch;
    sprite_index = spr_vigilante_getaway;
    image_index = 0;
}
...
if (((phase == 1 && elitehit <= 0) || (phase == 2 && elitehit <= 0)) && !pizzahead)
{
    if (phase == 1)
    {
        instance_destroy(obj_johnecheese, false);
