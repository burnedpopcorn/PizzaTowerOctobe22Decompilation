...
{
    var p1 = player;
    var p2 = (obj_player1.spotlight == true) ? 463 : 464;
...
if (room == strongcold_endscreen || room == Mainmenu || room == rm_levelselect || room == rank_room || room == timesuproom || room == Realtitlescreen || room == characterselect || room == hub_loadingscreen || (string_copy(room_get_name(room), 1, 5) == "tower" && !global.panic))
    visible = false;
else
    visible = true;
...
if (instance_exists(player) && !lock && player.state != states.timesup && player.state != states.gameover && !follow_golf && !detach)
{
    if (room != custom_lvl_room)
    {
        ...
        {
            var _targetcharge = player.xscale * ((abs(player.hsp) / 4) * 50);
            var _tspeed = 2;
