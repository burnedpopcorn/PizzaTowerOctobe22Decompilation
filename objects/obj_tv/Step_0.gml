...
if (room == Realtitlescreen || room == Mainmenu || room == rank_room || room == rm_levelselect || room == timesuproom || room == boss_room1 || room == characterselect || room == hub_loadingscreen)
{
    visible = false;
...
switch (state)
{
    case states.normal:
        ...
        if (!_transfo)
        {
            with (obj_player1)
            {
                ...
                {
                    if (global.combo >= 3 && global.stylethreshold < 3 && !obj_player.isgustavo)
                        tv_do_expression(spr_tv_exprcombo);
                    else if (global.stylethreshold >= 3 && !obj_player.isgustavo)
                        tv_do_expression(spr_tv_exprheat);
                }
                else if (!obj_player.isgustavo)
                    tv_do_expression(spr_tv_exprpanic);
            }
        }
    ...
    case states.transitioncutscene:
        if (sprite_index == spr_tv_open && ANIMATION_ENDED)
            sprite_index = tvsprite;
